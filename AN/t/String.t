...
is($an->String->get({key=>"t_0000"}), "Test", "get(); key only, hash reference-type call.");
is($an->String->get("t_0001", ["A"]), "Test replace: [A].", "get(); key and one variable, array type call.");
is($an->String->get({key=>"t_0001",variable=>["A"]}), "Test replace: [A].", "get(); key and one variable, hash reference type call.");
is($an->String->get({key=>"t_0002",variable=>["A", "B"]}), "Test Out of order: [B] replace: [A].", "get(); key and two variables, out of order injection.");
is($an->String->get({key=>"t_0002",variable=>["あ", "い"],language=>"jp"}), "テスト、 整理: [い]/[あ]。", "get(); key and two variables, out of order injection, alternate language.");
...
is($an->String->get({key=>"ta_0000",variable=>["A"],hash=>$test}), "Alternate Test: [A].", "get(); key and one variable, alternate words hash.");
$test={};	# Blank to test reload.
is($an->String->read_words({file=>"./test.xml", hash=>$test}), 1, "read_words(); Read in 'test.xml' into cleared hash reference for next set of tests, hash reference type call.");
is($an->String->read_words("./test.xml", $test), 1, "read_words(); Read in 'test.xml' into a new hash reference for next set of tests.");
is($an->String->get({key=>"ta_0000",variable=>["あ"],language=>"jp",hash=>$test}), "代りのテスト: [あ]。", "get(); key and one variable, alternate words hash and alternate language.");
is($an->String->get("ta_0000",["あ"],"jp",$test), "代りのテスト: [あ]。", "get(); key and one variable, alternate words hash and alternate language, array-type arguments.");
